...
class T3libParsehtmlProc {
	 ...
	 * @access private
	 */
	protected function get_tag_attributes_classic($tag, $deHSC = 0) {
	 ...
	 */
	protected function get_tag_attributes($tag, $deHSC = 0) {
		...
		if (is_array($components)) {
			foreach ($components as $key => $val) {
				if ($val != '=') { // Only if $name is set (if there is an attribute, that waits for a value), that valuemode is enabled. This ensures that the attribute is assigned it's value
					if ($valuemode) {
						if ($name) {
							$attributes[$name] = $deHSC ? self::htmlspecialchars_decode($val) : $val;
	 ...
	 */
	protected function split_tag_attributes($tag) {
		$matches = array();
